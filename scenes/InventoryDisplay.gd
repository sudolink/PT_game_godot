...
func get_item_description():
	if player_items and not slot_empty(selected_slot):
		description_box.show_description(player_items[selected_slot].get_child(0).description)
	else:
		description_box.show_description("Empty")
...
func highlight_icon():
	...
	if player_items and not slot_empty(previous_slot) and selected_slot != previous_slot:
...
func inventory_down():
	if selected_slot + 7 > max_slots-1:
		selected_slot += 7 - max_slots
	else:
		selected_slot += 7
	highlight()

func inventory_up():
	if selected_slot - 7 < 0:
		selected_slot += max_slots-7
	else:
		selected_slot -= 7
	highlight()
